...
private fun a(passports: List<Passport>) = passports.count { it.isValidA() }

private fun b(passports: List<Passport>) = passports.count { it.isValidB() }
...
private data class Passport(private val passportString: String) {
    ...
    fun isValidA() =  requiredFields.all { it in passportString }

    fun isValidB() = isValidA() && areAllValuesValid()

    private fun areAllValuesValid() = requiredFields.all { isFieldValid(it) }
...
private const val input = """byr:1971
iyr:2017 hgt:160cm
eyr:2020 ecl:hzl
pid:157096267
